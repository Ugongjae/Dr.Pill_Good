...
import base64
#import keras_shape, keras_color
...
def get_connection() :
    conn = pymysql.connect(host='127.0.0.1', user='root',
            password='dhahwl', db='pill_db', charset='utf8')
...
def test():
    ...
    print(mark)
        ...
        #    print(code[i], mark,color,shape)
    print(code)
        #return redirect('http://apis.data.go.kr/1470000/MdcinGrnIdntfcInfoService/getMdcinGrnIdntfcInfoList?serviceKey=Vlaoh%2BfNfPJgfZNGOaLA0FW7IVV%2F6QNRLyXSWcPbp4nIjuotGHYI8XgxDMEBgMqARSFgTIfNLPUt20cllzyunQ%3D%3D&item_seq='+str(code))
        #return 'uploads."
...
def upload_file():
    if request.method == 'POST':
        ...
        #print(f.filename)
        f.save("./test3/test_data/"+f.filename)
        #color = keras_color.predict()
        #shape = keras_shape.predict()
        color = keras_predict.color_predict()
        shape = keras_predict.shape_predict()
        os.remove("./test3/test_data/"+f.filename)
